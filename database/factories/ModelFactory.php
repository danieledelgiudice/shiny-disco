...
$factory->define(App\Cliente::class, function (Faker\Generator $faker) {
    ...
    return [
        ...
        'stato_pratica' => $faker->numberBetween(0, 3), //fixme
        'tipo_pratica' => $faker->numberBetween(0, 3), //fixme
        ...
        'medico_controparte' => $faker->optional($weight = 0.4)->name,
        
        'legale' => $faker->optional($weight = 0.3)->name,
        'in_data' => $faker->dateTimeThisDecade(),
        'controllato' => $faker->boolean,
        'data_ultima_lettera' => $faker->optional($weight = 0.4)->dateTimeThisDecade(),
        'mezzo_liquidabile' => $faker->randomElement(['']), //fixme
        'valore_mezzo_liquidabile' => $faker->randomFloat($nbMaxDecimals = 2, $min = 500, $max = 10000),
        'rilievi' => $faker->numberBetween(0, 2),
        'data_chiusura' => $faker->optional($weight = 0.3)->dateTimeThisDecade(),
        'importo_sospeso' => $faker->optional($weight = 0.3)->randomFloat($nbMaxDecimals = 2, $min = 500, $max = 3000),
        'data_sospeso' => $faker->optional($weight = 0.3)->dateTimeThisDecade(),
        'stato_avanzamento' => $faker->numberBetween(0, 8), //fixme
        
        'data_sinistro' => $faker->dateTimeThisDecade(),
        'ora_sinistro' => $faker->time,
        'luogo_sinistro' => $faker->city,
        'autorita' => $faker->numberBetween(0, 5), //fixme
        'comando_autorita' => $faker->optional($weight = 0.7)->state,
        'testimoni' => $faker->optional($weight = 0.3)->name,
        'rivalsa' => $faker->numberBetween(0, 4), //fixme
        'soccorso' => $faker->numberBetween(0, 4), //fixme
        'tipologia_intervento' => $faker->randomElement($tipologieIntervento),
        'danno_presunto' => $faker->optional(0.1)->randomFloat($nbMaxDecimals = 2, $min = 500, $max = 70000),
        'numero_sinistro' => strtoupper($faker->bothify('????##/####??##/?#??')),
        
        'assicurazione_risarcente' => $faker->company,
        'assicurazione_responsabile' => $faker->company,
        'mezzo_visibile' => $faker->optional($weight = 0.4)->randomElement(['Carrozzeria Tre Ponti', 'Carrozzeria Interdonato',
                                                                            'Previa telefonata al nostro assistito', 'Demolito']),
        'dinamica_sinistro' => $faker->optional($weight = 0.7)->randomElement($dinamiche),                                                                                                                                                              
